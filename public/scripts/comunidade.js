...
async function getposts(){
    try{
        const resultado = await fetch(base_url+"/get-posts-with-comments",{
            method: "GET",
            credentials: "include"
        });
...
async function getViewPost(){
    try{
        ...
        const responseExcluir = await fetch(base_url+"/viewExcluir");
...
async function postArea(){
    ...
    if(nomem != ""){
        ...
        postAreaSubmit.addEventListener("click", async () => {
            ...
            var img = "";
            if((postAreaInputImage.src + "") != "/comunidade"){
...
async function postPost(data){
    try {
        const resultado = fetch(base_url+"/post-post",{
            method: "POST",
            credentials: "include",
            headers:{
                "Content-Type": "Application/json"
